...
from ...data.forex_provider import fetch_forex_ohlcv
...
def _run_forecast_background(
    ...
    try:
        ...
        if market_type == "crypto":
            ...
            df = fetch_forex_ohlcv(symbol, timeframe)
            if df is None:
        ...
        result = run_forecast(
            ...
            is_high_impact_news_window=is_high_impact_news_window,
        )
        ...
        log.info(f"✓ Forecast complete for {symbol} {timeframe}: {result.get('final_signal')}")
...
def run_forecast_endpoint(
    ...
    news_sentiment: float = Query(0.0),
    is_high_impact_news_window: bool = Query(False),
):
    """
    ...
    background_tasks.add_task(
        _run_forecast_background, symbol, market_type, timeframe, exchange, news_sentiment, is_high_impact_news_window
    )
