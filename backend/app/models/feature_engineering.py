...
from .rules_strategy import TradingRulesStrategy
...
def add_features(df: pd.DataFrame, live_price: Optional[float] = None) -> pd.DataFrame:
    ...
    # Calculate required base technicals (ATR, RSI, S/R) before ML features
    ...
    # ATR (now calculated in TradingRulesStrategy)
    ...
    # RSI (now calculated in TradingRulesStrategy)
    ...
    # Support/Resistance (now calculated in TradingRulesStrategy)
    df["dist_to_support_atr"]    = (df["close"] - df.get("support_1", df["low"]))    / (df.get("atr_14", 1e-9) + 1e-9)
    df["dist_to_resistance_atr"] = (df.get("resistance_1", df["high"]) - df["close"]) / (df.get("atr_14", 1e-9) + 1e-9)
