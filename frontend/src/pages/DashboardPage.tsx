...
export const DashboardPage: React.FC<DashboardPageProps> = () => {
  ...
  if (forecast?.final_signal === 'LONG' || forecast?.final_signal === 'BUY') {
    headline = 'THE AI SEES\nAN UPWARD\nWINDOW.';
    signalClass = 'text-[var(--accent-fresh)]';
  } else if (forecast?.final_signal === 'SHORT' || forecast?.final_signal === 'SELL') {
    headline = 'THE AI SEES\nA DEFENSIVE\nWINDOW.';
              ...
              <div>
                <div className="flex items-baseline gap-4 mb-4">
                  <span
                    className={`font-display text-4xl md:text-5xl font-medium ${
                      forecast.final_signal === 'LONG' || forecast.final_signal === 'BUY'
                        ? 'text-[var(--accent-fresh)]'
                        : forecast.final_signal === 'SHORT' || forecast.final_signal === 'SELL'
                        ? 'text-[var(--red)]'
                  ...
                  <span className="font-mono text-2xl text-[var(--text-secondary)]">
                    {forecast.confidence_score ? `${Math.round(forecast.confidence_score * (forecast.confidence_score <= 1 ? 100 : 1))}%` : ''}
                  </span>
                </div>

                <p className="text-sm text-[var(--text-secondary)] mb-6 leading-relaxed">
                  "The model currently estimates a higher probability of{' '}
                  {forecast.final_signal === 'LONG' || forecast.final_signal === 'BUY' ? 'an upward move' : 'a downward or defensive move'}.
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-4 border-t border-[var(--border)]">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-xs text-[var(--text-primary)]">
                      {forecast.smc?.session || '24/7 Global'}
                    </span>
                  </div>
                  <div>
                    <span className="text-meta text-[var(--text-muted)] block mb-1">
